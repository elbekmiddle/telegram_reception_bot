...
async function manageCourses(
  ...
  while (true) {
    const total = await conversation.external(() => prisma.course.count())
    if (total === 0) {
  ...
  while (true) {
    ...
    if (data.startsWith('COURSE_EDIT|TITLE|')) {
      ...
      if (newTitle && newTitle !== course.title) {
        ...
        await ctx.reply(`✅ Nomi yangilandi: *${escapeMarkdown(newTitle)}*`, { parse_mode: 'Markdown' })
      } else if (newTitle === course.title) {
        await ctx.reply('ℹ️ Nom o\'zgarmadi.')
      }
      continue
    }
...
async function handleCourseCreate(
	...
	// ─── GUARD: 3x duplication fix ──────────────────────────────────────────
	// If courseId already exists in flowState (e.g. conversation replayed or
	// admin re-triggered the step), skip DB create and go straight to photo.
	if (state.data?.courseId) {
		ctx.session.flowState = { step: 'course_photo', data: { courseId: state.data.courseId } }
		return
	}

	// ─── Preview + confirm before creating ──────────────────────────────────
	// Shows admin a summary and asks to confirm — prevents accidental doubles.
	const previewText = [
		'📋 *Yangi kurs — tasdiqlash*',
		'',
		`🎓 *Nomi:* ${escapeMarkdown(state.data.title)}`,
		state.data.description ? `📝 *Tavsif:* ${escapeMarkdown(state.data.description)}` : '📝 *Tavsif:* —',
		`💰 *Narx:* ${state.data.price ?? '—'}`,
		'',
		'Yaratishni tasdiqlaysizmi?'
	].join('\n')

	const confirm = await askChoice(conversation, ctx, previewText, [
		{ text: '✅ Ha, yaratish', data: 'CONFIRM|YES' },
		{ text: '❌ Bekor qilish', data: 'CONFIRM|NO' }
	])

	if (!confirm || confirm === 'CONFIRM|NO') {
		await ctx.reply('❌ Kurs yaratish bekor qilindi.')
		ctx.session.flowState = { step: 'idle' }
		return
	}
	...
	// Store courseId IMMEDIATELY — guard above will catch any replays
	ctx.session.flowState = {
		step: 'course_photo',
		data: { ...state.data, courseId: course.id }
	}
	...
	// flowState already set above — do NOT set again here
}
